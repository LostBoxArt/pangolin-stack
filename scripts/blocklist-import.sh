#!/bin/bash
# ============================================================================
# CrowdSec Blocklist Import - Self-Contained Version
# ============================================================================
# Based on: https://github.com/wolffcatskyy/crowdsec-blocklist-import (MIT License)
# Imports 60k+ IPs from 28 public threat feeds into CrowdSec
#
# Usage: ./scripts/blocklist-import.sh
# Cron:  0 4 * * * /home/jesus/pangolin-stack/scripts/blocklist-import.sh
#
# Security notes:
# - No telemetry/phoning home (removed from original)
# - All filter commands are hardcoded, not user-supplied
...
setup_crowdsec() {
    ...
    if ! docker exec "$CROWDSEC_CONTAINER" cscli version &>/dev/null; then
        ...
        error "Check: docker ps | grep crowdsec"
        exit 1
    fi
}
...
fetch_list() {
    ...
    debug "Fetching $name..."
    if curl -sL --max-time 60 "$url" 2>/dev/null | eval "$filter" > "$output" 2>/dev/null; then
...
main() {
    ...
    # Censys (static list of known scanner ranges)
    cat <<EOF > censys.txt
192.35.168.0/23
162.142.125.0/24
74.120.14.0/24
...
EOF
    ...
    # Extract valid IPv4 addresses from all downloaded files
    cat *.txt 2>/dev/null | \
        grep -oE "[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+" | \
        awk -F'.' '{
    ...
    # Filter private/reserved/well-known DNS ranges
    grep -vE "^(10\.|172\.(1[6-9]|2[0-9]|3[01])\.|192\.168\.|127\.|0\.|255\.|100\.(6[4-9]|[7-9][0-9]|1[0-2][0-7])\.)" combined.txt | \
    grep -vE "^(1\.0\.0\.1|1\.1\.1\.1|8\.8\.8\.8|8\.8\.4\.4|9\.9\.9\.9|208\.67\.(222|220)\.(222|220))$" > filtered_private.txt || touch filtered_private.txt
    ...
    comm -23 filtered_private.txt existing.txt > to_import.txt || touch to_import.txt

    import_count=$(wc -l < to_import.txt 2>/dev/null || echo 0)
    total_ips=$(wc -l < filtered_private.txt 2>/dev/null || echo 0)

    if [[ $import_count -eq 0 ]]; then
        info "No new IPs to import (all $total_ips IPs already in CrowdSec)"
    else
        info "Importing $import_count new IPs into CrowdSec..."
        cat to_import.txt | run_cscli_stdin decisions import -i - --format values --duration "$DECISION_DURATION" --reason "external_blocklist" 2>&1
