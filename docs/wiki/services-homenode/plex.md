...
type: service
status: active
tags: ["homelab", "homenode", "service", "plex"]
aliases: ["plex"]
entities:
  primary: homenode-plex
  mentions: []
related: ["./README.md", "./homenode-review-2026-04-17.md", "./system-overview.md"]
sources: ["/volume1/docker/config/plex/docker-compose.yml", "host-configs/homenode/plex/docker-compose.yml"]
confidence: high
...
Plex Media Server. Serves `plex.example.com` (Traefik-routed) and direct
...
- **Tracked copy**: `host-configs/homenode/plex/docker-compose.yml` ✓
- **Port**: `32400:32400` (required for remote-access claim)
- **Router**: `plex.example.com`
- **Volumes**:
...
`plex.example.com` both still work. Only the "cast from a Smart TV by
