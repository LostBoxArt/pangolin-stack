...
type: service
status: active
tags: ["homelab", "vps", "service", "dashdot"]
aliases: ["dashdot"]
entities:
  primary: vps-dashdot
  mentions: []
...
Lightweight VPS system dashboard (CPU, RAM, storage, network). Runs behind
Traefik at `dash.dennisb.xyz`.
...
```59:79:stacks/observability/docker-compose.yml
  dashdot:
    ...
    labels:
      - "traefik.enable=true"
      - "traefik.http.routers.dashdot.rule=Host(`dash.dennisb.xyz`)"
