...
slug: vps-crowdsec-web-ui
type: service
status: active
tags: ["homelab", "vps", "service", "crowdsec-web-ui"]
aliases: ["crowdsec-web-ui"]
entities:
  primary: vps-crowdsec-web-ui
  mentions: []
...
the CrowdSec LAPI credentials. Acceptable for a VPS-admin tool reached over
...
`cswui.dennisb.xyz` (or sub-path) hostname with Pocket-ID auth:
...
```yaml
  crowdsec-web-ui:
    ...
    labels:
      - "traefik.enable=true"
      - "traefik.http.routers.cswui.rule=Host(`crowdsec-ui.dennisb.xyz`)"
