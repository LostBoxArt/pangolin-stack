---
title: "qbit-proxy"
slug: cloudnode-qbit-proxy
type: service
status: active
tags: ["homelab", "cloudnode", "service", "qbit-proxy"]
aliases: ["qbit-proxy"]
entities:
  primary: cloudnode-qbit-proxy
  mentions: []
...
- **Internal port**: `8081`
- **Target**: `torrent.example.com` (resolves via Homarr's `extra_hosts` to
  `192.168.1.10`)
...
2. Forwards them to `torrent.example.com` (i.e. qBit on HomeNode) with the
   correct `Host:` header.
...
```10:20:stacks/dashboard/docker-compose.yml
  qbit-proxy:
    ...
    environment:
      - TARGET_HOST=torrent.example.com
...
- If qBit on HomeNode changes hostname or IP, update both
