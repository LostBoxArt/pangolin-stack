...
type: service
status: active
tags: ["homelab", "nasus", "service", "traefik"]
aliases: ["traefik (NASUS)", "traefik"]
entities:
  primary: nasus-traefik
  mentions: []
related: ["./README.md", "./nasus-review-2026-04-17.md", "./system-overview.md"]
sources: ["/volume1/docker/traefik/docker-compose.yml", "host-configs/nasus/traefik/docker-compose.yml"]
confidence: high
...
Home-LAN reverse proxy for all NASUS services. Terminates TLS for
`*.dennisb.xyz` via Cloudflare DNS challenge. Distinct from the VPS Traefik.

- **Image**: `traefik:latest` ⚠️
- **Compose file**: `/volume1/docker/traefik/docker-compose.yml`
- **Tracked copy**: `host-configs/nasus/traefik/docker-compose.yml`
- **Ports**: `80`, `443`, **`9080:8080` (dashboard, insecure)** ⚠️
- **Networks**: `traefik_traefik` (external) — all NASUS services join it
...
services:
  traefik:
    ...
    command:
      ...
      - "--certificatesresolvers.cloudflare.acme.email=dennis@dennisb.xyz"
...
## Deviations from VPS Traefik

| Aspect | VPS | NASUS | Intent |
|---|---|---|---|
| Cert resolver | `letsencrypt` (HTTP-01) | `cloudflare` (DNS-01) | NASUS is LAN-only; DNS-01 avoids exposing port 80 to the internet |
| Dashboard | off | **on, insecure, published** | unintentional leak |
| docker.sock | read-write | **read-only** ✓ | NASUS is better here |
| Access logs | JSON, consumed by CrowdSec | none | no CrowdSec on NASUS |
...
`http://192.168.0.10:9080`. Discloses all routers, services,
entrypoints, middleware config, cert expiry. Any LAN device, any
container on the `traefik_traefik` network, and the VPS (through Olm)
can read it.

### F-N-TRAEFIK-2 — `:latest` image (`high` / NH2)
See VPS F-TRAEFIK-1 for rationale — same risk, different host.
...
NASUS host if you need dashboard access. Remove these lines:
...
```yaml
    ...
    labels:
      - "traefik.enable=true"
      - "traefik.http.routers.traefik.rule=Host(`traefik.dennisb.xyz`)"
...
Only worthwhile if you plan to deploy a local CrowdSec on NASUS.
...
- Cloudflare API token in `.env` needs `Zone:Read` + `DNS:Edit` on the
  `dennisb.xyz` zone.
- Public internet reaches NASUS services via Pangolin (VPS) → Newt tunnel,
  not via inbound 443. NASUS's `443:443` is only for LAN clients.
