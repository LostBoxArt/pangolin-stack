...
type: service
status: active
tags: ["homelab", "vps", "service", "linkstack"]
aliases: ["linkstack"]
entities:
  primary: vps-linkstack
  mentions: []
...
```11:35:stacks/apps/docker-compose.yml
  linkstack:
    ...
    group_add:
      - "986"  # docker group
    environment:
      - HTTP_SERVER_NAME=dennisb.xyz
      - HTTPS_SERVER_NAME=dennisb.xyz
      - LOG_LEVEL=info
      - PHP_MEMORY_LIMIT=256M
      - UPLOAD_MAX_FILESIZE=8M
      - TZ=Asia/Jerusalem
      - SERVER_ADMIN=admin@dennisb.xyz
    volumes:
      - linkstack_data:/htdocs
    labels:
      - "traefik.enable=true"
      - "traefik.http.routers.linkstack.rule=Host(`dennisb.xyz`)"
