---
title: "flaresolverr"
slug: homenode-flaresolverr
type: service
status: active
tags: ["homelab", "homenode", "service", "flaresolverr"]
aliases: ["flaresolverr"]
entities:
  primary: homenode-flaresolverr
  mentions: []
related: ["./README.md", "./homenode-review-2026-04-17.md", "./system-overview.md"]
sources: ["/volume1/docker/config/FlareSolverr/docker-compose.yml", "host-configs/homenode/FlareSolverr/docker-compose.yml"]
confidence: high
...
- **Compose file**: `/volume1/docker/config/FlareSolverr/docker-compose.yml`
  (note capital `F`)
- **Tracked copy**: `host-configs/homenode/FlareSolverr/docker-compose.yml` ✓
- **Port (internal)**: `8191`
- **Router**: `flaresolverr.example.com`
...
services:
  flaresolverr:
    ...
    labels:
      - "traefik.http.routers.flaresolverr.rule=Host(`flaresolverr.example.com`)"
...
Traefik exposes `flaresolverr.example.com`. The service is only ever
...
- FlareSolverr is resource-heavy (spawns a browser per request). If HomeNode
