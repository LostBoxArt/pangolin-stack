---
title: "HomeNode Compose-File Review — 2026-04-17"
slug: homenode-review-2026-04-17
type: review
status: active
tags: ["homelab", "homenode", "review", "audit"]
aliases: ["homenode audit 2026-04-17"]
entities:
  primary: homenode-review-2026-04-17
  mentions: []
related: ["./README.md", "./system-overview.md"]
sources: ["host-configs/homenode/", "/volume1/docker/"]
confidence: high
audience_level: operator
last_ingested: 2026-04-17
last_lint: 2026-04-17
---
# HomeNode Compose-File Review — 2026-04-17

Full audit of every `docker-compose.yml` under `/volume1/docker/` on HomeNode
(`192.168.1.10`) against upstream references. Findings, severities, and
remediation follow the same conventions as the
[CloudNode review](./compose-review-2026-04-17.md).
...
- Method: each compose file fetched over SSH from HomeNode and compared
  against upstream docs / reference compose / release notes.

Compose path convention on HomeNode:
...
`host-configs/homenode/` in this repo tracks only **8 of 16** services:
...
`AGENTS.md` currently lists 11 services for HomeNode; actual is 14 app services
...
- **Service**: [traefik](./services-homenode/traefik.md)
- **File**: `/volume1/docker/traefik/docker-compose.yml`
- **Symptom**: Traefik runs with `--api.insecure=true` and the container
  publishes `9080:8080` on the HomeNode host — meaning
  **`http://192.168.1.10:9080`** serves the full Traefik dashboard with no
  auth. Anyone on the home LAN (or reachable via the Olm tunnel — which
  includes the CloudNode) can read your routing table, cert expiry, entrypoints,
  middlewares, and provider credentials metadata.
- **Risk**: information disclosure; enables lateral-move planning by anyone
  on LAN or any compromised container on the `pangolin` CloudNode network.
...
- **Service**: [hawser](./services-homenode/hawser.md)
...
- **Symptom**: `TOKEN=${HAWSER_TOKEN:-}` with port `2375:2376` published on
  ...
  anyone who can reach HomeNode on TCP 2375 gets full Docker API access — root
...
- **Service**: [newt](./services-homenode/newt.md)
- **File**: `/volume1/docker/config/newt/docker-compose.yml`
- `NEWT_ID` and `NEWT_SECRET` are both in the compose file. Adding this
  file to git (as `host-configs/homenode/` does for tracked files) leaks the
  tunnel credentials. Neither value should ever live outside `.env`.

### NH2. `traefik:latest` on HomeNode
- Same pin-policy issue as CloudNode (H1 in CloudNode audit). HomeNode Traefik has no
  CrowdSec, no access-log consumer, so a bad `latest` pull would be
  noticed even later (no logs to stream).

### NH3. `dashdot` on HomeNode mounts the wrong path as host root
- **Service**: [dashdot](./services-homenode/dashdot.md)
- Volume is `/volume1/docker/config/dashdot:/mnt/host:ro` — that mounts
  ...
  HomeNode. The UI is essentially bogus.
- Upstream expects `/:/mnt/host:ro`.
- Also sets `privileged: true`, where on CloudNode we chose the safer
  `group_add: 986` pattern.

### NH4. `prowlarr:develop` (unstable channel)
- **Service**: [prowlarr](./services-homenode/prowlarr.md)
...
- **Service**: [qbittorrent](./services-homenode/qbittorrent.md)
...
- **Service**: [seerr](./services-homenode/seerr.md)
...
- 8 of 16 HomeNode compose files have no tracked copy in
  `host-configs/homenode/`. Listed in the repo-sync table above.
- Fix: copy the 8 missing files into `host-configs/homenode/<service>/docker-compose.yml`
  and write a short refresh script (e.g. `scripts/pull-homenode-configs.sh`) to
...
Every HomeNode service joins the same external network. qBit, Plex, and Hawser
...
which CrowdSec ingests. HomeNode Traefik has neither. All HomeNode services are
behind Cloudflare Tunnel + Pangolin, but a local CrowdSec at the HomeNode
...
`===END===` marker). Indicates the source file on HomeNode has no trailing
...
- **NL4**. Most services hard-code `TZ=Asia/Jerusalem` — fine, just flag
  for anyone moving HomeNode or using it across DST boundaries.
...
5. **NM1** — sync 8 missing files into `host-configs/homenode/`.
...
Each step = one commit (HomeNode-side change + updated `host-configs/homenode/`
...
HomeNode changes are applied by SSH'ing to HomeNode and editing the compose file
under `/volume1/docker/config/<service>/` then `docker compose -f ... up -d`.
Rollback:

```bash
ssh admin@192.168.1.10 "cd /volume1/docker/config/<service> && \
  sudo cp docker-compose.yml.bak docker-compose.yml && \
  docker compose up -d"
```

Always make a `.bak` before editing a HomeNode compose file. The repo copy
under `host-configs/homenode/` is *not* automatically redeployed — changes
flow **HomeNode → repo**, not the other way around, until you build a sync
script.
...
- 2026-04-17 — Added: initial HomeNode audit. 2 critical / 7 high / 13 medium /
  4 low findings. 8 of 16 services found to be untracked in
  `host-configs/homenode/` (no content drift on the 8 that are tracked).
