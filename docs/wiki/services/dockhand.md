---
title: "dockhand"
slug: cloudnode-dockhand
type: service
status: active
tags: ["homelab", "cloudnode", "service", "dockhand"]
aliases: ["dockhand"]
entities:
  primary: cloudnode-dockhand
  mentions: []
...
both for the CloudNode (local docker.sock) and HomeNode (remote Hawser agent over
TCP). Fronted at `dockhand.example.com`.
...
- **Bind**: `/opt/homelab/stacks:/opt/homelab/stacks`
...
```10:36:stacks/management/docker-compose.yml
  dockhand:
    ...
    labels:
      - "traefik.enable=true"
      - "traefik.http.routers.dockhand-redirect.rule=Host(`dockhand.example.com`)"
...
| Stacks bind mount | none | `/opt/homelab/stacks` | intentional — git stack deploys |
...
The `/opt/homelab/stacks:/opt/homelab/stacks`
...
- Public API: `https://dockhand.example.com/api/` — but for automation go
  through the container's internal IP on the CloudNode
  (`docker inspect -f '{{...}}' dockhand`) to skip Pocket-ID.
- Remote host **HomeNode** is registered with env `env=2`, host `192.168.1.10`,
