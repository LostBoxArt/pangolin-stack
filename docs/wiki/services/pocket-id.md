---
title: "pocket-id"
slug: cloudnode-pocket-id
type: service
status: active
tags: ["homelab", "cloudnode", "service", "pocket-id"]
aliases: ["pocket-id"]
entities:
  primary: cloudnode-pocket-id
  mentions: []
...
Traefik for admin UIs (`auth.example.com`).
...
```73:102:stacks/security/docker-compose.yml
  pocket-id:
    ...
    labels:
      - "traefik.enable=true"
      - "traefik.http.routers.pocketid-redirect.rule=Host(`auth.example.com`)"
      ...
      - "traefik.http.routers.pocketid.rule=Host(`auth.example.com`)"
