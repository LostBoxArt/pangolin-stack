---
title: "qbittorrent"
slug: nasus-qbittorrent
type: service
status: active
tags: ["homelab", "nasus", "service", "qbittorrent"]
aliases: ["qbittorrent"]
entities:
  primary: nasus-qbittorrent
  mentions: []
related: ["./README.md", "./nasus-review-2026-04-17.md", "./system-overview.md"]
sources: ["/volume1/docker/config/qbittorrent/docker-compose.yml", "host-configs/nasus/qbittorrent/docker-compose.yml"]
confidence: high
...
`torrent.dennisb.xyz` (Traefik-routed). **Runs with no VPN sidecar** —
...
- **Tracked copy**: `host-configs/nasus/qbittorrent/docker-compose.yml` ✓
- **Port (internal)**: `8080` (WEBUI_PORT)
- **Router**: `torrent.dennisb.xyz`
- **Volumes**:
...
services:
  qbittorrent:
    ...
    labels:
      - "traefik.http.routers.qbittorrent.rule=Host(`torrent.dennisb.xyz`)"
...
qBit runs directly on the NASUS network namespace. All outbound torrent
...
on the VPS (fixed via `qbit-proxy`) — another reason to know exactly which
version is running.
...
- **Homarr widget from the VPS** uses the `qbit-proxy` sidecar on the VPS
  (see VPS [qbit-proxy](../services/qbit-proxy.md)) because of the
  `Secure` cookie issue introduced in qBit 5.1.4. Do NOT point Homarr
  directly at `https://torrent.dennisb.xyz`.
