...
**Objective:** Configure `nasus` (Home Server, 192.168.0.10) to expose its Docker API over TCP so the VPS can monitor its containers via the Dashboard.

**Current Status:**
- VPS has a secure tunnel to `nasus` (via Olm/WireGuard).
- VPS can ping `192.168.0.10`.
- VPS fails to connect to Docker API: `Connection reset by peer` on port 2375.
- This indicates Docker on `nasus` is installed but NOT configured to accept remote TCP connections.
...
### Security Note
Since `nasus` is behind a secure VPN/WireGuard tunnel (not exposed to the public internet) and only accessible via the trusted tunnel interface, exposing unencrypted Docker socket on port 2375 is acceptable for this internal monitoring use case.
