...
type: service
status: active
tags: ["homelab", "nasus", "service", "hawser"]
aliases: ["hawser"]
entities:
  primary: nasus-hawser
  mentions: []
...
Dockhand's remote agent. The VPS Dockhand connects to this agent over TCP
to manage NASUS containers in the same UI as VPS containers.
...
- **Port**: `2375:2376` (Standard mode — Dockhand initiates from VPS)
...
- Note image org is **`finsys`** (with "i"), while Dockhand on VPS is
...
services:
  hawser:
    ...
    environment:
      ...
      # - DOCKHAND_SERVER_URL=wss://dockhand.dennisb.xyz/api/hawser/connect
...
For a home NAS reachable only via Olm tunnel from the VPS, **Edge mode is
...
`2375` published on `0.0.0.0`, this is full remote root on NASUS.
...
For NASUS, which is only reachable from the VPS via Olm tunnel, Edge mode
...
One of the 8 missing files in `host-configs/nasus/`. Given this file
...
…but this only works if Dockhand reaches NASUS from the host's own
loopback, which it does NOT (it comes from 192.168.0.1 via the Olm
...
This removes the public listen port on NASUS entirely. The agent connects
outbound to the VPS Dockhand, which authenticates via the token during
...
Dockhand version you're running on the VPS:
...
`host-configs/nasus/hawser/docker-compose.yml` and commit.

## Operational Notes

- Dockhand-side registration: Environments → New → "Hawser Standard" →
  host `192.168.0.10`, port `2375`, token matches `.env`.
- VPS-to-NASUS network path requires the Olm tunnel (`192.168.0.0/24 dev
  olm`) up. If `docker exec dockhand curl http://192.168.0.10:2375/...`
  hangs, check `olm-watchdog.timer` on the VPS first.
